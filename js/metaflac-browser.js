/*!
 * generated by gpt-5
 * metaflac-js 浏览器版（改造版）
 * 将 Node 依赖（fs, Buffer, file-type, image-size）替换为浏览器实现
 * 仅支持以 ArrayBuffer/Uint8Array 作为输入（不再接受文件路径字符串）
 *
 * 使用示例:
 *   const flac = new MetaFlac(arrayBuffer);
 *   flac.setTag("TITLE=测试");
 *   await flac.importPictureFromBuffer(pictureArrayBuffer);
 *   const out = flac.save(); // Uint8Array
 */
...
(function (global) {
    ...
    // expose to global
    global.MetaFlac = MetaFlac;
    global.BrowserBuffer = BrowserBuffer;
